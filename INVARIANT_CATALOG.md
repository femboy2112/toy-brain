...
50 fixtures total. I-CAT-01 is enforced at runner entry; its catalog fixture column is `_meta`. The Phase 3.1, Phase 3.2, Phase 3.3, Phase 3.4, Operator TUI, and Phase 3.5 fixture files are introduced incrementally by their campaigns; until their implementation steps land, explicit pending registrations in `brain/invariants.py` keep catalog coverage coherent without claiming the rows green.
...
- **REQUIRED v0.12 invariants:** 139
- **STRUCTURAL (constructor- or type-enforced, not per-tick asserted):** 48
- **NOT-EXERCISED row-level:** 5 (plus 5 modules covered at module-level in "Modules with no v0-required invariants")
- **DEFERRED row-level:** 12 (plus inherited deferrals table)
- **OBSERVED row-level:** 8 (recorded in run summary, not gating)

Total tabular entries: 212. v0.12 success is gated by the 139 REQUIRED rows + 48 STRUCTURAL rows (OBSERVED rows are logged but do not gate; the I-CAT-01 runner audit gates separately at startup).
