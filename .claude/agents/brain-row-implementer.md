...
- `INVARIANT_CATALOG.md` - row definitions, fixtures, status. Current baseline
  is v0.8 unless the file on disk says otherwise: 105 REQUIRED, 29 STRUCTURAL,
  3 NOT-EXERCISED, 12 DEFERRED, 4 OBSERVED.
...
1. **Look up the row(s).** Use `python3 -m tools.catalog show I-XXX-NN` and
   `python3 -m tools.catalog list --id-prefix I-XXX` to confirm fixture
...
5. **Run the runner.** `python3 -m brain.invariants run --id I-XXX` to
