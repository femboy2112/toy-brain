...
|---|---|---|---|---|
| `I-PSPEECH-20` | `I-PSPEECH-STRICT-COUNTER` | Proto-speech strict counter present (the headline fix that motivates Phase 3.33). | `stable_combination_count_strict` on `ProtoSpeechAcquisitionReport`, computed from the same evidence as the graceful `stable_combination_count` with acceptance narrowed to the canonical `STABLE_COMBINATION` disposition. [^pspeech20-f2] | STRUCTURAL |

[^pspeech20-f2]: The bundle's original design doc also proposed `transfer_success_count_strict`. The Step 4 strictness audit (Finding F-2) **falsified** that hypothesis: `proto_speech_acquisition.HOLOPHRASE_TRANSFER`'s runner is already strict (`transfers != 1` → FAIL; negative-context `disposition is TRANSFERRED` → FAIL), and the existing `transfer_success_count` report field is a turn-flag count, not a disposition count — so a "strict variant" would be a different measurement, not a narrowed version of the same one. No `transfer_success_count_strict` field is added; `I-PSPEECH-20` covers `stable_combination_count_strict` alone. See `PHASE3_33_STRICTNESS_AUDIT_PLAN.md` Finding F-2 for the full reasoning.
