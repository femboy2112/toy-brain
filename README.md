# brain — TLICA-constrained Python kernel (catalog v0.26)
...
Use `INVARIANT_CATALOG.md` as shipped. Version banner inside should say **v0.26**. Confirmation numbers: **282 REQUIRED · 89 STRUCTURAL · 14 NOT-EXERCISED · 15 DEFERRED · 16 OBSERVED · 166 fixtures** (Phase 3.18 Bounded Internal Processing Window catalog patch added `I-PWND-01..02` and two new fixtures: `processing_window_static_audit.py` covering `I-PWND-02` and `processing_window_integration.py` covering `I-PWND-01`; both rows participate in I-CAT-01 coverage. The Phase 3.15 L1 Cache Hygiene additions and the Phase 3.14 LLM Cache Discipline additions are unchanged.). Run `python3 -m tools.catalog counts` to verify; the strict gate fails if banner / actual / expected ever drift. If you see anything that looks like 74 REQUIRED, 92 REQUIRED, float+EPS, or `Literal[...]` for `Act`, that is an older draft and is wrong.
...
For catalog v0.26, the expected count is:
**282 REQUIRED · 89 STRUCTURAL · 14 NOT-EXERCISED · 15 DEFERRED · 16 OBSERVED**.
