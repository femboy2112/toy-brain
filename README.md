# brain — TLICA-constrained Python kernel (catalog v0.25)
...
Use `INVARIANT_CATALOG.md` as shipped. Version banner inside should say **v0.25**. Confirmation numbers: **281 REQUIRED · 88 STRUCTURAL · 14 NOT-EXERCISED · 15 DEFERRED · 16 OBSERVED · 164 fixtures** (Phase 3.15 L1 Cache Hygiene catalog patch promoted `I-LLMCACHE-20` from DEFERRED to REQUIRED, added the new `llm_cache_l1_capacity.py` fixture covering `I-LLMCACHE-20/23/24/25`, and added `I-LLMCACHE-26` to the existing `llm_cache_static_audit.py`. `I-LLMCACHE-21` and `I-LLMCACHE-22` remain NOT-EXERCISED in v1 and none of those two participates in I-CAT-01 coverage). Run `python3 -m tools.catalog counts` to verify; the strict gate fails if banner / actual / expected ever drift. If you see anything that looks like 74 REQUIRED, 92 REQUIRED, float+EPS, or `Literal[...]` for `Act`, that is an older draft and is wrong.
...
For catalog v0.25, the expected count is:
**281 REQUIRED · 88 STRUCTURAL · 14 NOT-EXERCISED · 15 DEFERRED · 16 OBSERVED**.
