# brain — TLICA-constrained Python kernel (catalog v0.28)
...
Use `INVARIANT_CATALOG.md` as shipped. Version banner inside should say **v0.28**. Confirmation numbers: **284 REQUIRED · 91 STRUCTURAL · 14 NOT-EXERCISED · 15 DEFERRED · 16 OBSERVED · 170 fixtures** (Phase 3.20 Coherence Feedback Bridge catalog patch added `I-CFBK-01..02` and two new fixtures: `coherence_feedback_static_audit.py` covering `I-CFBK-02` and `coherence_feedback_integration.py` covering `I-CFBK-01`; both rows participate in I-CAT-01 coverage. The Phase 3.19 Internal Feedback Loop additions, the Phase 3.18 Bounded Internal Processing Window additions, the Phase 3.15 L1 Cache Hygiene additions, and the Phase 3.14 LLM Cache Discipline additions are unchanged.). Run `python3 -m tools.catalog counts` to verify; the strict gate fails if banner / actual / expected ever drift. If you see anything that looks like 74 REQUIRED, 92 REQUIRED, float+EPS, or `Literal[...]` for `Act`, that is an older draft and is wrong.
...
For catalog v0.28, the expected count is:
**284 REQUIRED · 91 STRUCTURAL · 14 NOT-EXERCISED · 15 DEFERRED · 16 OBSERVED**.
