"""Phase 3.21 milestone M03_RECOGNITION fixture.

Drives I-DEVMILE-03 (REQUIRED). The fixture invokes
`run_m03_recognition` from the milestone harness with two distinct
...
defined in PHASE3_21_TEN_MILESTONES.md Section 3.
"""
...
@register("I-DEVMILE-03", status="REQUIRED")
def check_developmental_milestone_m03() -> None:
    """Phase 3.21 milestone M03_RECOGNITION integration audit."""
    for seed_offset in (0, 7):
        result = run_m03_recognition(seed_offset=seed_offset)
        assert result.milestone == (
            DevelopmentalMilestone.M03_RECOGNITION
        ), (
            "I-DEVMILE-03 violated: milestone mismatch "
            f"(got {result.milestone!r})"
        )
        assert result.status == MilestoneStatus.PASS, (
            "I-DEVMILE-03 violated: status "
            f"{result.status.value!r} != pass (summary: {result.summary!r})"
        )
        assert isinstance(result.summary, str) and result.summary.isprintable(), (
            "I-DEVMILE-03 violated: summary not bounded printable"
        )
        assert len(result.summary) <= MILESTONE_SUMMARY_MAX_LEN, (
            "I-DEVMILE-03 violated: summary length "
            f"{len(result.summary)} exceeds MILESTONE_SUMMARY_MAX_LEN"
        )
        assert COGITO_ID not in result.summary, (
            "I-DEVMILE-03 violated: summary contains COGITO_ID"
        )
        term = _has_forbidden_term(result.summary)
        assert term is None, (
            "I-DEVMILE-03 violated: summary contains forbidden "
            f"non-claim term {term!r}"
        )
        result2 = run_m03_recognition(seed_offset=seed_offset)
        assert result == result2, (
            "I-DEVMILE-03 violated: helper not deterministic for "
