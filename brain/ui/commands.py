...
class Command:
    ...
    Drives ``I-UI-03``. The constructor enforces:

    * ``kind`` is an :class:`OperatorCommand` enum member.
    * ``payload`` is exactly one of: ``None`` (for any command kind
      that does not take a payload), or :class:`QueuePerceptPayload`
    ...
    payload: Optional[QueuePerceptPayload] = None
...
def make_command(
    ...
    *,
    ...
    if resolved is OperatorCommand.QUEUE_PERCEPT:
        ...
        payload: Optional[QueuePerceptPayload] = QueuePerceptPayload(
